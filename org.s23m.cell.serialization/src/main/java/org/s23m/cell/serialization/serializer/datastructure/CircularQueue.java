...
package org.s23m.cell.serialization.serializer.datastructure;
...
public final class CircularQueue extends AbstractCollection {
	...
	public CircularQueue(final int c) {
		this(c, MAX_CAPACITY);
	}

	public CircularQueue(final int c, final int mc) {
		if (c > mc) {
	...
	private CircularQueue(final CircularQueue oldQueue) {
		size = oldQueue.size;
	...
	public String toString() {
		final StringBuffer s = new StringBuffer(super.toString() + " - capacity: '" +
				capacity() + "' size: '" + size() + "'");
