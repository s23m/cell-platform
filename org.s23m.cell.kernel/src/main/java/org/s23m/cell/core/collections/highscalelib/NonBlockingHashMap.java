package org.s23m.cell.core.collections.highscalelib;
 ...
 *
 * See https://raw.githubusercontent.com/boundary/high-scale-lib/master/src/main/java/org/cliffc/high_scale_lib/NonBlockingHashMap.java
 */
...
implements ConcurrentMap<TypeK, TypeV>, Cloneable, Serializable {
	...
	static {                      // <clinit>
		...
		catch( final java.lang.NoSuchFieldException e ) { throw new RuntimeException(e); }
	...
	private static final class Prime {
		final Object _V;
		Prime( final Object V ) { _V = V; }
		static Object unbox( final Object V ) { return V instanceof Prime ? ((Prime)V)._V : V; }
	}
	...
	private static final CHM   chm   (final Object[] kvs) { return (CHM  )kvs[0]; }
	private static final int[] hashes(final Object[] kvs) { return (int[])kvs[1]; }
	// Number of K,V pairs in the table
	private static final int len(final Object[] kvs) { return (kvs.length-2)>>1; }
	...
	private static final Object key(final Object[] kvs,final int idx) { return kvs[(idx<<1)+2]; }
	private static final Object val(final Object[] kvs,final int idx) { return kvs[(idx<<1)+3]; }
	private static final boolean CAS_key( final Object[] kvs, final int idx, final Object old, final Object key ) {
		return _unsafe.compareAndSwapObject( kvs, rawIndex(kvs,(idx<<1)+2), old, key );
	}
	private static final boolean CAS_val( final Object[] kvs, final int idx, final Object old, final Object val ) {
	...
	private final void print( final Object[] kvs ) {
		for( int i=0; i<len(kvs); i++ ) {
			final Object K = key(kvs,i);
			if( K != null ) {
				final String KS = (K == TOMBSTONE) ? "XXX" : K.toString();
				final Object V = val(kvs,i);
				final Object U = Prime.unbox(V);
				final String p = (V==U) ? "" : "prime_";
				final String US = (U == TOMBSTONE) ? "tombstone" : U.toString();
				System.out.println(""+i+" ("+KS+","+p+US+")");
			}
		}
		final Object[] newkvs = chm(kvs)._newkvs; // New table, if any
		if( newkvs != null ) {
			System.out.println("----");
			print(newkvs);
		}
	...
	private final void print2( final Object[] kvs) {
		for( int i=0; i<len(kvs); i++ ) {
			final Object key = key(kvs,i);
			final Object val = val(kvs,i);
			final Object U = Prime.unbox(val);
			if( key != null && key != TOMBSTONE &&  // key is sane
					val != null && U   != TOMBSTONE ) { // val is sane
				final String p = (val==U) ? "" : "prime_";
	...
	public long reprobes() { final long r = _reprobes.get(); _reprobes = new Counter(); return r; }
	...
	private static final int reprobe_limit( final int len ) {
	...
	@Override
	public boolean containsKey( final Object key )            { return get(key) != null; }
	...
	public boolean contains   ( final Object val )            { return containsValue(val); }
	...
	@Override
	public TypeV   put        ( final TypeK  key, final TypeV val ) { return putIfMatch( key,      val, NO_MATCH_OLD); }
	 ...
	 *  @throws NullPointerException if the specified key or value is null  */
	public TypeV   putIfAbsent( final TypeK  key, final TypeV val ) { return putIfMatch( key,      val, TOMBSTONE   ); }
	 ...
	 *  @throws NullPointerException if the specified key is null */
	@Override
	public TypeV   remove     ( final Object key )            { return putIfMatch( key,TOMBSTONE, NO_MATCH_OLD); }
	 ...
	 *  @throws NullPointerException if the specified key or value is null */
	public boolean remove     ( final Object key,final Object val ) {
	 ...
	 *  @throws NullPointerException if the specified key or value is null */
	public TypeV   replace    ( final TypeK  key, final TypeV val ) { return putIfMatch( key,      val,MATCH_ANY   ); }
	 ...
	 *  @throws NullPointerException if the specified key or value is null */
	public boolean replace    ( final TypeK  key, final TypeV  oldValue, final TypeV newValue ) {
		final Object out = putIfMatch(key, newValue, oldValue);
		return (oldValue == null) ? out == oldValue : oldValue.equals(out);
	}

	private final TypeV putIfMatch( final Object key, final Object newVal, final Object oldVal ) {
	...
	@Override
	public void putAll(final Map<? extends TypeK, ? extends TypeV> m) {
		for (final Map.Entry<? extends TypeK, ? extends TypeV> e : m.entrySet()) {
	...
	public void clear() {         // Smack a new empty table down
		final Object[] newkvs = new NonBlockingHashMap(MIN_SIZE)._kvs;
	...
	public Object clone() {
		try {
			...
			final NonBlockingHashMap<TypeK,TypeV> t = (NonBlockingHashMap<TypeK,TypeV>) super.clone();
	...
	public String toString() {
		final Iterator<Entry<TypeK,TypeV>> i = entrySet().iterator();
		if( !i.hasNext()) {
			return "{}";
		}
		...
		sb.append('{');
		for (;;) {
			final Entry<TypeK,TypeV> e = i.next();
			final TypeK key = e.getKey();
			final TypeV value = e.getValue();
	...
	// 'equals' v-call.
	private static boolean keyeq( final Object K, final Object key, final int[] hashes, final int hash, final int fullhash ) {
		return
	...
	@Override
	public TypeV get( final Object key ) {
	...
	// copies in progress.
	private final Object[] help_copy( final Object[] helper ) {
		...
		final Object[] topkvs = _kvs;
		final CHM topchm = chm(topkvs);
	...
	private static final class CHM<TypeK,TypeV> {
		...
		boolean CAS_newkvs( final Object[] newkvs ) {
			while( _newkvs == null ) {
		...
		private final boolean tableFull( final int reprobe_cnt, final int len ) {
			return
		...
		private final Object[] resize( final NonBlockingHashMap topmap, final Object[] kvs) {
			assert chm(kvs) == this;
			...
			final int oldlen = len(kvs);    // Old count of K,V pairs allowed
			final int sz = size();          // Get current table count of active K,V pairs
			...
			final int megs = ((((1<<log2)<<1)+4)<<3/*word to bytes*/)>>20/*megs*/;
			if( r >= 2 && megs > 0 ) { // Already 2 guys trying; wait and see
				...
				try { Thread.sleep(8*megs); } catch( final Exception e ) { }
			}
		...
		// of the top position.
		private final void help_copy_impl( final NonBlockingHashMap topmap, final Object[] oldkvs, final boolean copy_all ) {
			assert chm(oldkvs) == this;
			final Object[] newkvs = _newkvs;
			assert newkvs != null;    // Already checked by caller
			final int oldlen = len(oldkvs); // Total amount to copy
		...
		// read it yet.
		private final Object[] copy_slot_and_check( final NonBlockingHashMap topmap, final Object[] oldkvs, final int idx, final Object should_help ) {
			assert chm(oldkvs) == this;
			final Object[] newkvs = _newkvs; // VOLATILE READ
		...
		private final void copy_check_and_promote( final NonBlockingHashMap topmap, final Object[] oldkvs, final int workdone ) {
			assert chm(oldkvs) == this;
			final int oldlen = len(oldkvs);
		...
		private boolean copy_slot( final NonBlockingHashMap topmap, final int idx, final Object[] oldkvs, final Object[] newkvs ) {
			...
			final Object old_unboxed = ((Prime)oldval)._V;
			assert old_unboxed != TOMBSTONE;
			final boolean copied_into_new = (putIfMatch(topmap, newkvs, key, old_unboxed, null) == null);
	...
	private class SnapshotV implements Iterator<TypeV>, Enumeration<TypeV> {
		...
		Object key(final int idx) { return NonBlockingHashMap.key(_sskvs,idx); }
	...
	public Collection<TypeV> values() {
		return new AbstractCollection<TypeV>() {
			...
			@Override public boolean contains( final Object v ) { return NonBlockingHashMap.this.containsValue(v); }
	...
	public Set<TypeK> keySet() {
		return new AbstractSet<TypeK> () {
			@Override public void    clear   (          ) {        NonBlockingHashMap.this.clear   ( ); }
			@Override public int     size    (          ) { return NonBlockingHashMap.this.size    ( ); }
			@Override public boolean contains( final Object k ) { return NonBlockingHashMap.this.containsKey(k); }
			@Override public boolean remove  ( final Object k ) { return NonBlockingHashMap.this.remove  (k) != null; }
