 ...
 *
 * See https://raw.githubusercontent.com/boundary/high-scale-lib/master/src/main/java/org/cliffc/high_scale_lib/ConcurrentAutoTable.java
 */
...
public class ConcurrentAutoTable implements Serializable {
	 ...
	 */
	public void add( final long x ) { add_if_mask(  x,0); }
	/** {@link #add} with -1 */
	...
	public void set( final long x ) {
		final CAT newcat = new CAT(null,4,x);
	...
	private long add_if_mask( final long x, final long mask ) { return _cat.add_if_mask(x,mask,hash(),this); }
	...
	private boolean CAS_cat( final CAT oldcat, final CAT newcat ) { return _catUpdater.compareAndSet(this,oldcat,newcat); }
	...
	private static class CAT implements Serializable {
		...
		private static long rawIndex(final long[] ary, final int i) {
			assert i >= 0 && i < ary.length;
			return _Lbase + i * _Lscale;
		}
		private final static boolean CAS( final long[] A, final int idx, final long old, final long nnn ) {
		...
		CAT( final CAT next, final int sz, final long init ) {
			_next = next;
		...
		// ONCE.
		public long add_if_mask( final long x, final long mask, final int hash, final ConcurrentAutoTable master ) {
			final long[] t = _t;
			final int idx = hash & (t.length-1);
			// Peel loop; try once fast
			long old = t[idx];
			final boolean ok = CAS( t, idx, old&~mask, old+x );
			...
			if( (r>>17) != 0 ) {      // Already too much allocation attempts?
				...
				try { Thread.sleep(r>>17); } catch( final InterruptedException e ) { }
				if( master._cat != this ) {
					return old;
				}
			}

			final CAT newcat = new CAT(this,t.length*2,0);
		...
		// the cache.
		public long estimate_sum( final long mask ) {
		...
		public void all_or ( final long mask ) {
			final long[] t = _t;
		...
		public void all_and( final long mask ) {
			final long[] t = _t;
			for( int i=0; i<t.length; i++ ) {
				boolean done = false;
				while( !done ) {
		...
		public void all_set( final long val ) {
			final long[] t = _t;
			for( int i=0; i<t.length; i++ ) {
				t[i] = val;
			}
		...
		String toString( final long mask ) { return Long.toString(sum(mask)); }
