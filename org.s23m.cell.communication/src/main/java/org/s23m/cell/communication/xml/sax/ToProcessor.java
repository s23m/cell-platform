...
public class ToProcessor extends AbstractAttributeAwareProcessor<Node> {

	@Override
	public Node startElement(Namespace namespace, XmlSchemaTerminology terminology, Node top, Attributes attributes) {
		// ambiguity - which "to" are we talking about?
		if (top instanceof Visibility || top instanceof SuperSetReference) {
			...
			return new ToIdentityReference(namespace, terminology, uniqueRepresentationReference, identifier, null);
	...
	public void endElement(Node removed, Node top, String textContent) {
		if (top instanceof Visibility) {
			((Visibility) top).setTo((ToIdentityReference) removed);
		} else if (top instanceof SuperSetReference) {
			((SuperSetReference) top).setTo((ToIdentityReference) removed);
		} else if (top instanceof Edge) {
			((Edge) top).setTo((EdgeEnd) removed);
		} else {
			throw new IllegalStateException("To element could not be terminated: " + removed);
		}
	}
